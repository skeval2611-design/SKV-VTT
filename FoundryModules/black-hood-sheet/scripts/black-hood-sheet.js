...
Hooks.once('init', function() {
  // Проверяем систему (рекомендуем Simple Worldbuilding)
  ...
  // Добавляем тип актёра 'blackHood' в систему
  const systemData = game.system.model.Actor;
  if (!systemData.types.includes('blackHood')) {
    systemData.types.push('blackHood');
  }

  // Расширяем базовый класс Actor для типа blackHood (задаём значения по умолчанию)
  ...
  class BlackHoodSheet extends ActorSheet {
    ...
    // Обработчик кликов на статы для броска кубов
    activateListeners(html) {
      super.activateListeners(html);

      // Бросок куба при клике на значение стата
      html.find('.stat-value').click(this._onStatRoll.bind(this));

      // Переключение хайлайта при клике на кнопку хайлайта
      html.find('.highlight-toggle').click(this._onToggleHighlight.bind(this));

      // Кнопки увеличения/уменьшения Honour, HP, XP
  ...
  // Регистрируем лист для типа blackHood
